...
import { getNextActions } from "@/lib/nextAction";
...
export async function GET(req: NextRequest) {
  try {
    ...
    // Rate limiting
    const rateLimitResult = await checkRateLimit(`next-action:${decoded.uid}`);
    ...
    const actions = await getNextActions(decoded.uid);
    ...
    console.error("[NEXT_ACTION] unexpected error:", error);
